...
void ModemRTTY::setBaud(double baud)
{
    ...
    if (getInternalState() != INTSTATE_PREINIT)
        restart();
}
...
{
    if (getInternalState() == INTSTATE_TX_STARTING) {
        // check if transmission has just started and send preamble
    ...
    // TX buffer empty
    if (getInternalState() == INTSTATE_TX_STOPPING) {
        //m_stopFlag = false;
        //line_char_count = 0;
        if (m_bits != 5) {
    ...
    QChar qtCharacter;
    bool hasChar = getNextChar(qtCharacter);
    char character = qtCharacter.toLatin1();

    // there is no character to send, so just send idle signals
    if (!hasChar) {
        sendIdle();
        return;
    }
    ...
    int baudotCharacter = baudotEnc(character);
    if (baudotCharacter < 0) {
        //return 0;
        return;
    }

    // switch case if necessary
    if ((baudotCharacter & 0x300) != m_txMode) {
...
double ModemRTTY::computeMetric() const
{
...
void ModemRTTY::sendChar(int c)
{
    ...
    if (m_bits == 5) {
        ...
        if (c)
            ...
            emit sent('?'); // unregognized character
    }
    else
        emit sent(c);
}
...
void SymbolShaper::preset(double baud, double sr)
{
    ...
    for (int x = 0; x < m_tableSize; ++x ) {
        ...
        double const T = wfactor * m_sampleRate / (m_baudRate * 2.0);   // symbol-length in samples if wmultiple = 1.0
